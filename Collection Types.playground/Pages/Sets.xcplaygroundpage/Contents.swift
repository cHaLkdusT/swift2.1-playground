...
Because `Hashable` protocol conforms to `Equatable`, conforming types must also provide an implementation of the "is equal" operator (`==`)
...
letters.insert("a")
letters = []
//: ### Creatin a Set with an Array Literal
...
//: A set type cannot be inferred from an array literal alon, so the type `Set` must be explicitly declared. However, because of Swift's type inference, you don't have to write the type of the set if you're initializing it with an array literal containing values of the same type
...
//: Switf's `Set` type does not have a defined ordering. To iterte over the values of a set in a specific order, use the `sort()` method, which returns the set's element as an array sorted using the `<` operator
...
/*:
* `intersect(_:)` method to create a new set with only the values common to both sets
* `exclusiveOr(_:)` method to create a new set with values in either set, but not both
* `union(_:)` method to create a new set with all of the values in both sets
* `substract(_:)` method to create a new set with values not in the specifie set.
*/
...
* `isSubsetOf(_:)` method to determine whether all of the values of a set are contained in the specified set
* `isSupersetOf(_:)` method to determin whether a set contains all of the values in a specified set
* `isStrictSubsetOf(_:)` or `isStrictSupersetOf(_:)` methods to determine whether a set is a subtset or superset, but not equal to, a specified set
* `isDisjointWith(_:)` method to determine wheter two sets have any values in commont
*/
