...
//: Switch
/*: 
A `switch` statement considers a value and compares it agains several possible matching patterns. It then executes an appropriate block of code, based on the first pattern that matches successfully. A `switch` statement provides an alternative to the `if` statement for responding to multiple potential states
...
//}
/*: 
Unlike a `switch` statement in C, this `switch` statement does not both `"a"` and `"A"`. Rather, it reports a compile-time error that `case "a":` does not contain any executable statements. This approach avaoid accidefaul fallthrough from one case to another, and makes for safer code that is clearer in its intent.
...
//: The `switch` statement determins if the point is at the origin (0, 0); on the red x-axis; on the orange y-axis; inside the blue 4-by-4 box centered on the origin; or outside of the box
//: ### Value Bindings
